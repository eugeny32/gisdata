...
let PointAttributes_rslib_entry_i = 0;
for(const obj in PointAttributeTypes){
    PointAttributeTypes[PointAttributes_rslib_entry_i] = PointAttributeTypes[obj];
    PointAttributes_rslib_entry_i++;
}
...
class PointAttributes {
    constructor(pointAttributes){
        ...
        this.size = 0;
        this.vectors = [];
        if (null != pointAttributes) for(let i = 0; i < pointAttributes.length; i++){
    ...
    hasNormals() {
        for(const name in this.attributes){
            ...
            if (pointAttribute === PointAttribute.NORMAL_SPHEREMAPPED || pointAttribute === PointAttribute.NORMAL_FLOATS || pointAttribute === PointAttribute.NORMAL || pointAttribute === PointAttribute.NORMAL_OCT16) return true;
        }
        return false;
    }
}
export { PointAttribute, PointAttributeTypes, PointAttributes };
