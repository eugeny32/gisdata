class Version {
    constructor(version){
        ...
        let vmLength = -1 === version.indexOf('.') ? version.length : version.indexOf('.');
...
onmessage = function(event) {
    ...
    for (let pointAttribute of pointAttributes.attributes){
        if ("POSITION_CARTESIAN" === pointAttribute.name) {
        ...
        } else if ("rgba" === pointAttribute.name) {
        ...
        } else if ("NORMAL_SPHEREMAPPED" === pointAttribute.name) {
            let buff = new ArrayBuffer(12 * numPoints);
        ...
        } else if ("NORMAL_OCT16" === pointAttribute.name) {
            let buff = new ArrayBuffer(12 * numPoints);
            let normals = new Float32Array(buff);
            for(let j = 0; j < numPoints; j++){
                let bx = view.getUint8(inOffset + j * pointAttributes.byteSize + 0);
        ...
        } else if ("NORMAL" === pointAttribute.name) {
            let buff = new ArrayBuffer(12 * numPoints);
            let normals = new Float32Array(buff);
            for(let j = 0; j < numPoints; j++){
            ...
            if (pointAttribute.type.size > 4) {
                ...
                if (null != pointAttribute.initialRange) {
