function readUsingDataView(event) {
    ...
    let pBuff = new ArrayBuffer(12 * numPoints);
    let cBuff = new ArrayBuffer(4 * numPoints);
    let iBuff = new ArrayBuffer(4 * numPoints);
    let clBuff = new ArrayBuffer(numPoints);
    let rnBuff = new ArrayBuffer(numPoints);
    let nrBuff = new ArrayBuffer(numPoints);
    let psBuff = new ArrayBuffer(2 * numPoints);
    ...
    const rangeReturnNumber = [
        1 / 0,
        -1 / 0
    ];
    const rangeNumberOfReturns = [
        1 / 0,
        -1 / 0
    ];
    const rangeSourceID = [
    ...
    for(let i = 0; i < numPoints; i++){
        ...
        let returnNumber = 7 & returnNumberAndNumberOfReturns;
        let numberOfReturn = (56 & returnNumberAndNumberOfReturns) >> 3;
    ...
    let indices = new ArrayBuffer(4 * numPoints);
    let iIndices = new Uint32Array(indices);
    for(let i = 0; i < numPoints; i++)iIndices[i] = i;
