async function readUsingDataView(event) {
    ...
    const buffer = isFullFile ? await Copc.Las.PointData.decompressFile(u) : await Copc.Las.PointData.decompressChunk(u, {
    ...
    const buffers = {
        position: new ArrayBuffer(12 * pointCount),
        color: new ArrayBuffer(6 * pointCount),
        intensity: new ArrayBuffer(4 * pointCount),
        ...
        pointSourceId: new ArrayBuffer(2 * pointCount),
        gpsTime: new ArrayBuffer(4 * pointCount),
        indices: new ArrayBuffer(4 * pointCount)
    };
    const tempBuffers = {
        gpsTime64: new ArrayBuffer(8 * pointCount),
        color16: new ArrayBuffer(6 * pointCount)
    };
    const views = {
    ...
    const get = {
        ...
        ...view.dimensions.GpsTime && {
            gpsTime: view.getter('GpsTime')
        },
        ...view.dimensions.Red && {
            red: view.getter('Red'),
