...
public:
  ...
  {
    // Было: tf::TransformBroadcaster (публикация текущей позы для RViz).
    // Headless-сборка без визуализации -- намеренный no-op.
    (void)xc;
  }
  ...
  void pub_global_path(vector<vector<ScanPose*>*> &relc_bl_buf, NullPublisher &pub_relc, vector<int> &ids)
  {
  ...
  void pub_globalmap(vector<vector<Keyframe*>*> &relc_submaps, vector<int> &ids, NullPublisher &pub)
  {
    pcl::PointCloud<pcl::PointXYZI> pl;
    pub_pl_func(pl, pub);
    pcl::PointXYZI pp;
...
public:
  ...
  void previous_map_names(const ParamReader &n, vector<string> &fnames, vector<double> &juds)
  {
    string premap;
  ...
  void previous_map_read(vector<STDescManager*> &std_managers, vector<vector<ScanPose*>*> &multimap_scanPoses, vector<vector<Keyframe*>*> &multimap_keyframes, ConfigSetting &config_setting, PGO_Edges &edges, const ParamReader &n, vector<string> &fnames, vector<double> &juds, string &savepath, int win_size)
  {
    int acsize = 10; int mgsize = 5;
    n.param<int>("Loop/acsize", acsize, 10);
    n.param<int>("Loop/mgsize", mgsize, 5);

    for(int fn=0; fn<fnames.size(); fn++)
    {
...
public:
  ...
  void motion_blur(pcl::PointCloud<PointType> &pl, PVec &pvec, IMUST xc, IMUST xl, deque<ImuSample::Ptr> &imus, double pcl_beg_time, IMUST &extrin_para)
  {
    ...
    {
      ImuSample &head = **(it_imu-1);
      ImuSample &tail = **(it_imu); 
  ...
  int motion_init(vector<pcl::PointCloud<PointType>::Ptr> &pl_origs, vector<deque<ImuSample::Ptr>> &vec_imus, vector<double> &beg_times, Eigen::MatrixXd *hess, LidarFactor &voxhess, vector<IMUST> &x_buf, unordered_map<VOXEL_LOC, OctoTree*> &surf_map, unordered_map<VOXEL_LOC, OctoTree*> &surf_map_slide, vector<PVecPtr> &pvec_buf, int win_size, vector<vector<SlideWindow*>> &sws, IMUST &x_curr, deque<IMU_PRE*> &imu_pre_buf, IMUST &extrin_para)
  {
    ...
    for(int iterCnt = 0; iterCnt < 10; iterCnt++)
    {
      ...
      if(fabs(resis[0] - resis[1]) / resis[0] < converge_thre && iterCnt >= 2)
      {
        ...
        is_degrade = eigvalue[0] < 8 ? true : false;
...
public:
  ...
  VOXEL_SLAM(const ParamReader &n)
  {
    ...
    // Было: sub_imu/sub_pcl = n.subscribe(...) -- живая подписка на ROS-топики.
    // Эта сборка ВСЕГДА offline (см. offline_bag_open/offline_bag_feed в
    // voxelslam.hpp) -- imu_handler/pcl_handler вызываются оттуда напрямую,
    // подписка не нужна вообще.
    ...
    int ss = 0;
    // POSIX access(path, X_OK) как проверка "директория существует" ->
    // Windows-эквивалент _access(path, 0) (F_OK), из <io.h>. На нативной
    // Linux-сборке -- прямой вызов настоящего POSIX access()/mkdir(), без
    // обёртки: это ровно то, чем является сам Windows-шим ниже, просто без
    // необходимости в шиме вообще.
#ifdef _WIN32
    if(_access((savepath+bagname+"/").c_str(), 0) == -1)
    {
      // Было system("mkdir " + path) -- эта команда идёт через ВСТРОЕННЫЙ
      // MKDIR у cmd.exe (не Win32 API), а тот, в отличие от fopen/ifstream
      // в остальном коде, вообще не понимает прямые слэши в пути (даже
      // один-единственный -- "mkdir D:\foo\bar/" тоже падает) -- эмпирически
      // подтверждено: alidarState.txt никогда не писался именно из-за этого
      // (директория тихо не создавалась, ofstream в неё дальше тоже тихо
      // проваливался). _mkdir() из <direct.h> — обычная CRT-функция, прямые
      // слэши ей не мешают, никакого отдельного процесса-обёртки не нужно.
      ss = _mkdir((savepath+bagname+"/").c_str());
    }
    else
      ss = -1;
#else
    if(access((savepath+bagname+"/").c_str(), F_OK) == -1)
    {
      ss = mkdir((savepath+bagname+"/").c_str(), 0777);
    }
    else
      ss = -1;
#endif
  ...
  int initialization(deque<ImuSample::Ptr> &imus, Eigen::MatrixXd &hess, LidarFactor &voxhess, PLV(3) &pwld, pcl::PointCloud<PointType>::Ptr pcl_curr)
  {
    static vector<pcl::PointCloud<PointType>::Ptr> pl_origs;
    static vector<double> beg_times;
    static vector<deque<ImuSample::Ptr>> vec_imus;
  ...
  void system_reset(deque<ImuSample::Ptr> &imus)
  {
    for(auto iter=surf_map.begin(); iter!=surf_map.end(); iter++)
    {
      iter->second->tras_ptr(octos_release);
  ...
  void thd_odometry_localmapping(const ParamReader &n)
  {
    PLV(3) pwld;
    ...
    vector<deque<ImuSample::Ptr>> vec_imus;
    bool release_flag = false;
    int degrade_cnt = 0;
    LidarFactor voxhess(win_size);
    const int mgsize = 1;
    Eigen::MatrixXd hess;
    long progress_iter = 0;
    while(true)
    {
      // Периодический health-сигнал (Фаза 3) -- headless-процесс без
      // ROS/RViz иначе не даёт вообще никакой видимости в состояние
      // одометрического потока при батч-запуске из PHP/Node-оркестрации;
      // недорого (раз в 2000 итераций) и не влияет на алгоритм.
      if((++progress_iter % 2000) == 1)
        printf("[progress] iter=%ld pcl_buf=%zu imu_buf=%zu imu_last_time=%lf g_bag_done=%d\n",
               progress_iter, pcl_buf.size(), imu_buf.size(), imu_last_time, (int)g_bag_done);
      // ros::spinOnce() removed -- no live ROS topics in this offline build
      ...
      // Было n.setParam("finish",true)/n.param<bool>("finish",is_finish,false)
      // -- избыточный обход через ROS parameter server, is_finish уже
      // обычное поле класса, читаемое напрямую другими потоками
      // (thd_loop_closure/thd_globalmapping), см. их break-условия ниже.
      if(is_finish)
      {
        break;
      }

      deque<ImuSample::Ptr> imus;
      ...
      else
      {
        ...
        rtk_datum_accumulate(x_curr.t, x_curr.p);
        ...
        printf("[trace] recut win=%d\n", win_count); fflush(stdout);
        multi_recut(surf_map_slide, win_count, x_buf, voxhess, sws);
        printf("[trace] recut_done win=%d\n", win_count); fflush(stdout);
        t3 = now_sec();
  ...
  void thd_loop_closure(const ParamReader &n)
  {
      ...
      if(isOpt)
      {
        ...
        for(int i=subsize-init_num; i<subsize; i++)
        {
          ...
          for(PointType &ap: sp.plptr->points)
          {
            ...
            // Было ap.normal[j] (см. тот же комментарий про PCL data[4] выше)
            pv.var(0, 0) = ap.normal_x;
            pv.var(1, 1) = ap.normal_y;
            pv.var(2, 2) = ap.normal_z;
    ...
    // ИЗМЕНЕНО (не в оригинале): было "if(is_save_map)" — в исходном ROS-коде
    // это осмысленно (is_save_map=0 = чисто интерактивный запуск, смотрим
    // через RViz-паблишеры, на диск вообще ничего не пишем). У нашего
    // headless offline-инструмента (Фаза 3 плана переноса) паблишеры —
    // no-op-заглушки, поэтому alidarState.txt (траектория, единственный
    // РЕАЛЬНЫЙ результат всего прогона для compute_slam.mjs) — НЕ опция, а
    // обязательный вывод, независимо от is_save_map. is_save_map оставлен
    // только за тем, за чем и был -- дамп per-frame PCD ниже
    // (win_base+i savepath+bagname, см. is_save_map в главном цикле выше) —
    // это дорого (тысячи файлов на длинной записи) и реально опционально,
    // его продолжаем гейтить отдельно.
    {
      for(int i=0; i<(int)multimap_scanPoses.size(); i++)
        FileReaderWriter::instance().save_pose(*(multimap_scanPoses[i]), sessionNames[i], "/alidarState.txt", savepath);
  ...
  void thd_globalmapping(const ParamReader &n)
  {
    ...
    // ДОБАВЛЕНО (не в оригинале): страховка от вечного зависания этого
    // потока. total_ba==1 (единственный путь к `if(is_finish) break;` ниже)
    // требует gba_flag==1 && smp_mp>=cnct_map.back() && gba_size<=buf_base --
    // в оригинале это было безопасно ждать бесконечно (живой ROS-процесс,
    // завершаемый Ctrl+C снаружи), но в offline-пакетном режиме запись без
    // реального loop closure (короткий скан/вырожденная геометрия -- ровно
    // случай, на котором это найдено при первом сквозном тесте на
    // синтетических данных) может никогда не удовлетворить это условие,
    // и поток будет вечно поллить sleep(0.1) уже ПОСЛЕ того, как is_finish
    // выставлен. idle_after_finish считает итерации, где мы НЕ сделали
    // реальной работы (застряли в одной из sleep-ветвей) после is_finish;
    // после ~10с (100 * 0.1с) такого простоя выходим без финального
    // GBA-прохода -- то же самое компромиссное решение, что и деградация на
    // вендорский frame_pose.txt при недоступности SLAM (см. план) --
    // частичный/неполированный результат лучше зависшего процесса.
    int idle_after_finish = 0;
    const int idle_after_finish_limit = 100;

    while(true)
    {
      if(multimap_keyframes.empty())
      {
        if(is_finish && ++idle_after_finish > idle_after_finish_limit) break;
        sleep(0.1); continue;
      }
      ...
      if(total_ba == 1)
      {
        ...
        // ДОБАВЛЕНО (не в оригинале): в оригинале этот поток жил, пока жив
        // весь ROS-процесс (n.ok()), останавливаясь только внешним Ctrl+C
        // -- имело смысл в живом/интерактивном режиме, но не для offline-
        // пакетной обработки, где процесс должен завершиться сам. Момент,
        // когда total_ba==1 (полный/финальный GBA-проход) сработал ПРИ
        // уже выставленном is_finish, -- надёжный признак, что это
        // последний проход по всей записи (total_ba триггерится только
        // когда smp_mp догнал последний connected map), поэтому здесь
        // безопасно выйти из потока.
        if(is_finish) break;
      }
...
// Было: ros::init + ros::NodeHandle (конфиг через rosparam, обычно
// загружаемый `rosparam load config.yaml` до старта ноды). Здесь вместо
// этого — один аргумент командной строки с путём к плоскому текстовому
// конфигу (см. compat/param_reader.hpp — не YAML, см. его докстринг про
// сломанный yaml-cpp) — тот же принцип, что pdal.exe/untwine.exe уже
// вызываются из PHP-оркестрации (bin/slam_step_worker.ps1 -> node.exe
// slam/steps/computeSlam.mjs готовит и путь к конфигу, и путь к
// промежуточному файлу вместо bag_path/lid_topic/imu_topic).
int main(int argc, char **argv)
{
  // stdout к файлу/пайпу (не к консоли) по умолчанию буферизуется ПОЛНОСТЬЮ
  // (десятки КБ), а не построчно -- при батч-запуске из PHP/Node-оркестрации
  // (лог всегда идёт в файл) это означало, что многочасовой прогресс не
  // виден в логе вплоть до самого выхода процесса. Построчная буферизация
  // делает progress-принты (per-scan/loop/GBA) видимыми в реальном времени
  // (ПРИМЕЧАНИЕ: на Windows CRT для файлового/pipe-вывода это не всегда
  // помогает -- см. отладку в истории коммитов -- но не вредит и корректно
  // работает при запуске в консоли, оставлено).
...

  {
    ...
    n.param<std::string>("intermediate_path", intermediate_path, "");
    if(!intermediate_path.empty()) offline_bag_open(intermediate_path);
    std::string rtk_path;
    n.param<std::string>("rtk_path", rtk_path, std::string(""));
    if(!rtk_path.empty()) rtk_load(rtk_path);
    n.param<double>("gps_wh", g_gps_wh, 0.0);
    n.param<double>("gps_wv", g_gps_wv, 0.0);
  }
  mp = new int[vs.win_size];
  for(int i=0; i<vs.win_size; i++)
    mp[i] = i;

  int loop_enable_flag = 1;
  n.param<int>("Loop/enable", loop_enable_flag, 1);
  thread thread_loop;
  if(loop_enable_flag) thread_loop = thread(&VOXEL_SLAM::thd_loop_closure, &vs, ref(n));
  ...
  thread_gba.join();
  // Было ros::spin() -- всё реальное содержательное завершается ДО этой
  // точки (все три потока уже join()'ed), в оригинале это тоже был
  // мёртвый код к этому моменту при работе из bag (см. Фазу 0 плана).
  return 0;
}
