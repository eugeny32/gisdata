...
public:
  ...
  void motion_blur(IMUST &xc, pcl::PointCloud<PointType> &pcl_in, deque<ImuSample::Ptr> &imus)
  {
    ...
    {
      ImuSample &head = **(it_imu);
      ImuSample &tail = **(it_imu+1);
    ...
    ImuSample::Ptr imu1(new ImuSample(*imus.front()));
    ImuSample::Ptr imu2(new ImuSample(*imus.back()));
  ...
  void IMU_init(deque<ImuSample::Ptr> &imus)
  {
    Eigen::Vector3d cur_acc, cur_gyr;
    for(ImuSample::Ptr imu: imus)
    {
  ...
  int process(IMUST &x_curr, pcl::PointCloud<PointType> &pcl_in, deque<ImuSample::Ptr> &imus)
  {
    if(!init_flag)
    {
      IMU_init(imus);
