...
(admin-only gate onto the CtF·ADMIN Node oversight service), plus the
three session-persistence JSON APIs (api/facade_cad_session.php/
api/topo_cad_session.php/api/facade_foto_session.php) -- see
cad_sessions/services.py for the shared EntityJSON<->PostGIS conversion.
"""
...
def facade_cad_view(request):
    ...
    if not admin and not _service_enabled(user, "facade_cad_enabled"):
        return HttpResponse(
            "403 Forbidden: услуга FACADE·CAD не подключена для вашего аккаунта — обратитесь к администратору.",
            status=403,
        )
    return render(request, "cad_sessions/iframe_tool.html", {
        "frame_id": "facadeCadFrame", "frame_src": "/assets/facade-cad/index.html",
        "frame_title": "FACADE·CAD", "frame_icon": "bi-rulers",
    })
...
def topo_cad_view(request):
    ...
    if not admin and not _service_enabled(user, "topo_cad_enabled"):
        return HttpResponse(
            "403 Forbidden: услуга TOPO·CAD не подключена для вашего аккаунта — обратитесь к администратору.",
            status=403,
        )
    return render(request, "cad_sessions/iframe_tool.html", {
        "frame_id": "topoCadFrame", "frame_src": "/assets/topo-cad/index.html",
        "frame_title": "TOPO·CAD", "frame_icon": "bi-signpost-2",
    })
...
def facade_foto_view(request):
    ...
    if not admin and not _service_enabled(user, "facade_foto_enabled"):
        return HttpResponse(
            "403 Forbidden: услуга FACADE·FOTO не подключена для вашего аккаунта — обратитесь к администратору.",
            status=403,
        )
    return render(request, "cad_sessions/iframe_tool.html", {
        "frame_id": "facadeFotoFrame", "frame_src": "/assets/facade-foto/index.html",
        "frame_title": "FACADE·FOTO", "frame_icon": "bi-image",
    })
...
def ctfadmin_view(request):
    ...
    if not ctf_pass:
        return HttpResponse("500: CTFADMIN_ADMIN_PASS не задан в .env", status=500)
