...
from tours.models import Tour, TourFile
        ...
        for relative_path in ply_files:
            ...
            if current_size is None:
                ...
                if lock_age_min > STARTUP_GRACE_MINUTES:
                    stalled = True
                    self.stderr.write(f"Завис (нет выходного файла спустя {round(lock_age_min)} мин): {relative_path}")
            elif prev is not None and current_size <= prev["size"] and (time.time() - prev["at"]) / 60 > STALL_MINUTES:
                stalled = True
                self.stderr.write(f"Завис (выходной файл не растёт дольше {STALL_MINUTES} мин): {relative_path}")
            ...
            if retries + 1 >= MAX_STALL_RETRIES:
                Path(str(base) + ".sog.error").write_text(
                    f"Конвертация зависала {retries} раз подряд (сервер перезапускался/процесс убит без завершения) "
                    "— автоматические попытки остановлены, нужно разобраться вручную.",
                    encoding="utf-8",
                )
                retries_file.unlink(missing_ok=True)
                self.stderr.write(f"{relative_path}: превышен лимит автоповторов ({retries}), помечен как .error")
            else:
                retries_file.write_text(str(retries + 1))
                self.stdout.write(f"{relative_path}: lock снят, будет перезапущен (попытка {retries + 1}/{MAX_STALL_RETRIES})")

        running_count = sum(
            1 for relative_path in ply_files
            if Path(str(upload_dir / relative_path) + ".sog.lock").is_file()
        )
        self.stdout.write(f"PLY-файлов в БД: {len(ply_files)}, уже запущенных конвертаций (lock): {running_count}")

        spawned = 0
        for relative_path in ply_files:
            if running_count + spawned >= MAX_CONCURRENT:
                self.stdout.write(f"Достигнут лимит параллельных конвертаций ({MAX_CONCURRENT}) — остальное в следующий запуск.")
                break
            ...
            spawned += 1
            self.stdout.write(f"Запущена конвертация: {relative_path} -> {relative_path}.sog + .collision.glb")

        self.stdout.write(f"Запущено новых конвертаций: {spawned}")
