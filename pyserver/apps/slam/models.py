...
in PIPELINE_ORDER (see services.py, Р¤Р°Р·Р° 7 of the migration plan) вЂ”
...
class SlamProject(models.Model):
    ...
    # (needed for local, non-EPSG projections вЂ” e.g. vendor FusionCRS_TM_87
...
class SlamScan(models.Model):
    ...
    # accurate enough even after outlier filtering + rigid alignment вЂ” better
    ...
    # Not always a real EPSG code вЂ” the auto-computed UTM zone is a bare
...
class SlamJob(models.Model):
    ...
    # Heartbeat updated by the worker while running вЂ” same stale-job
