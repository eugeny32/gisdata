...
def slam_projects_view(request):
    ...
    if request.method == "POST":
        ...
        if action == "save":
            ...
            if not name:
                error = "РЈРєР°Р¶РёС‚Рµ РЅР°Р·РІР°РЅРёРµ РїСЂРѕРµРєС‚Р°"
            else:
                if project_id > 0:
...
def slam_scans_view(request):
    ...
    if request.method == "POST":
        ...
        if action == "upload":
            ...
            if not name:
                error = "РЈРєР°Р¶РёС‚Рµ РЅР°Р·РІР°РЅРёРµ СЃРєР°РЅР°"
            elif not uploaded and not existing_file_name:
                error = "Р—Р°РіСЂСѓР·РёС‚Рµ ZIP-Р°СЂС…РёРІ (bag + calibration.yaml) РёР»Рё СѓРєР°Р¶РёС‚Рµ РёРјСЏ СѓР¶Рµ Р·Р°РіСЂСѓР¶РµРЅРЅРѕРіРѕ С„Р°Р№Р»Р°"
            else:
                ...
                if uploaded:
                    ...
                    if not incoming_path.is_file():
                        error = f'Р¤Р°Р№Р» "{existing_file_name}" РЅРµ РЅР°Р№РґРµРЅ РІ uploads/slam/_incoming/ РЅР° СЃРµСЂРІРµСЂРµ'
                    else:
