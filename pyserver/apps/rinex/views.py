...
def _validate_form(selected_stations, date_from_str, date_to_str):
    if not selected_stations:
        return None, None, "Р’С‹Р±РµСЂРёС‚Рµ С…РѕС‚СЏ Р±С‹ РѕРґРЅСѓ СЃС‚Р°РЅС†РёСЋ"
    if not date_from_str or not date_to_str:
        return None, None, "РЈРєР°Р¶РёС‚Рµ РїРµСЂРёРѕРґ (UTC)"
    try:
        date_from = datetime.fromisoformat(date_from_str)
        date_to = datetime.fromisoformat(date_to_str)
    except ValueError:
        return None, None, "РќРµРєРѕСЂСЂРµРєС‚РЅР°СЏ РґР°С‚Р°/РІСЂРµРјСЏ"
    if date_from > date_to:
        return None, None, "РќР°С‡Р°Р»Рѕ РїРµСЂРёРѕРґР° РїРѕР·Р¶Рµ РєРѕРЅС†Р°"
    days = (date_to.date() - date_from.date()).days + 1
    if days > RINEX_MAX_DAYS:
        return None, None, f"РЎР»РёС€РєРѕРј С€РёСЂРѕРєРёР№ РґРёР°РїР°Р·РѕРЅ ({days} РґРЅРµР№) вЂ” РјР°РєСЃРёРјСѓРј {RINEX_MAX_DAYS} РґРЅРµР№ Р·Р° РѕРґРёРЅ СЂР°Р·, СЃСѓР·СЊС‚Рµ РїРµСЂРёРѕРґ"
...
def rinex_view(request):
    ...
    if intent == "create_request":
        ...
        if not error and not selected_types:
            error = "Р’С‹Р±РµСЂРёС‚Рµ С…РѕС‚СЏ Р±С‹ РѕРґРёРЅ С‚РёРї С„Р°Р№Р»РѕРІ"
        if not error:
        ...
        if not error:
            ...
            if ftp is None:
                error = "РќРµ СѓРґР°Р»РѕСЃСЊ РїРѕРґРєР»СЋС‡РёС‚СЊСЃСЏ Рє FTP-СЃРµСЂРІРµСЂСѓ GNSS-РґР°РЅРЅС‹С… (gnss.host) вЂ” РїРѕРїСЂРѕР±СѓР№С‚Рµ РїРѕР·Р¶Рµ"
            else:
                try:
...
def rinex_download_zip_view(request):
    ...
    if not paths:
        return HttpResponseBadRequest("РќРµ РІС‹Р±СЂР°РЅРѕ РЅРё РѕРґРЅРѕРіРѕ С„Р°Р№Р»Р°")
    if len(paths) > RINEX_ZIP_MAX_FILES:
        return HttpResponseBadRequest(f"РЎР»РёС€РєРѕРј РјРЅРѕРіРѕ С„Р°Р№Р»РѕРІ Р·Р° РѕРґРёРЅ СЂР°Р· (РјР°РєСЃРёРјСѓРј {RINEX_ZIP_MAX_FILES})")
    ...
    for p in paths:
        if not _VALID_ZIP_PATH_RE.match(p):
            return HttpResponseBadRequest("РќРµРґРѕРїСѓСЃС‚РёРјС‹Р№ РїСѓС‚СЊ С„Р°Р№Р»Р°")

    ftp = gnss_ftp_connect()
    if ftp is None:
        return HttpResponse("РќРµ СѓРґР°Р»РѕСЃСЊ РїРѕРґРєР»СЋС‡РёС‚СЊСЃСЏ Рє FTP-СЃРµСЂРІРµСЂСѓ GNSS-РґР°РЅРЅС‹С…", status=502)
    ...
    if not zip_path.is_file() or zip_path.stat().st_size == 0:
        ...
        tmp_dir.rmdir()
        return HttpResponse("РќРµ СѓРґР°Р»РѕСЃСЊ СЃРѕР±СЂР°С‚СЊ Р°СЂС…РёРІ вЂ” РЅРё РѕРґРёРЅ С„Р°Р№Р» РЅРµ СЃРєР°С‡Р°Р»СЃСЏ СЃ FTP", status=502)
...
def rinex_request_download_view(request):
    ...
    if not rr or not rr.result_path:
        return HttpResponseNotFound("Р—Р°РїСЂРѕСЃ РЅРµ РЅР°Р№РґРµРЅ РёР»Рё РµС‰С‘ РЅРµ РіРѕС‚РѕРІ")

    full_path = Path(settings.UPLOADS_ROOT) / "rinex_results" / rr.result_path
    if not full_path.is_file():
        return HttpResponseNotFound("Р¤Р°Р№Р» СЂРµР·СѓР»СЊС‚Р°С‚Р° РЅРµ РЅР°Р№РґРµРЅ РЅР° СЃРµСЂРІРµСЂРµ")
...
def rgen_parse_stations_input(text: str) -> list[dict]:
    ...
    for raw_line in _re.split(r"\r\n|\r|\n", text.strip()):
        ...
        if len(parts) < 4:
            raise ValueError(f'РЎС‚СЂРѕРєР° "{line}" вЂ” РЅСѓР¶РЅРѕ 4 РїРѕР»СЏ: РРјСЏ, X, Y, Z')
        name, x, y, z = parts[0], parts[1], parts[2], parts[3]
        try:
            xf, yf, zf = float(x), float(y), float(z)
        except ValueError:
            raise ValueError(f'РќРµРєРѕСЂСЂРµРєС‚РЅР°СЏ СЃС‚СЂРѕРєР° СЃС‚Р°РЅС†РёРё: "{line}"')
        if not name:
            raise ValueError(f'РќРµРєРѕСЂСЂРµРєС‚РЅР°СЏ СЃС‚СЂРѕРєР° СЃС‚Р°РЅС†РёРё: "{line}"')
        stations.append({"name": name, "ecef": (xf, yf, zf)})
    if not stations:
        raise ValueError("РЈРєР°Р¶РёС‚Рµ С…РѕС‚СЏ Р±С‹ РѕРґРЅСѓ СЃС‚Р°РЅС†РёСЋ")
    return stations
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if end_unix <= start_unix:
                raise ValueError("РљРѕРЅРµС† РїРµСЂРёРѕРґР° РґРѕР»Р¶РµРЅ Р±С‹С‚СЊ РїРѕР·Р¶Рµ РЅР°С‡Р°Р»Р°")
            if end_unix - start_unix > 7 * 86400:
                raise ValueError("РњР°РєСЃРёРјР°Р»СЊРЅС‹Р№ РїРµСЂРёРѕРґ РіРµРЅРµСЂР°С†РёРё вЂ” 7 СЃСѓС‚РѕРє (РёРЅР°С‡Рµ С„Р°Р№Р» Р±СѓРґРµС‚ РѕРіСЂРѕРјРЅС‹Рј, Р° СЂР°СЃС‡С‘С‚ вЂ” СЃР»РёС€РєРѕРј РґРѕР»РіРёРј)")
            ...
            if missing_days:
                raise RuntimeError(
                    "РќРµ СѓРґР°Р»РѕСЃСЊ Р°РІС‚РѕРјР°С‚РёС‡РµСЃРєРё Р·Р°РіСЂСѓР·РёС‚СЊ СЌС„РµРјРµСЂРёРґС‹ Р·Р°: " + ", ".join(missing_days) + ". "
                    "РџСЂРѕРІРµСЂСЊС‚Рµ РґРѕСЃС‚СѓРї СЃРµСЂРІРµСЂР° РІ РёРЅС‚РµСЂРЅРµС‚ РґРѕ РёСЃС‚РѕС‡РЅРёРєРѕРІ РёР· РЅР°СЃС‚СЂРѕРµРє "
                    "(RINEX_SYNTH_NAV_URL_TEMPLATES), Р»РёР±Рѕ РїСЂРёР»РѕР¶РёС‚Рµ navigation-С„Р°Р№Р» РІСЂСѓС‡РЅСѓСЋ РЅРёР¶Рµ Рё "
                    "РїРѕРІС‚РѕСЂРёС‚Рµ (СЂРµР·РµСЂРІ СЂР°Р±РѕС‚Р°РµС‚ С‚РѕР»СЊРєРѕ РґР»СЏ РїРµСЂРёРѕРґР° РІ РїСЂРµРґРµР»Р°С… РѕРґРЅРёС… СЃСѓС‚РѕРє)."
                )

            eph = rgen_merge_ephemerides(day_files)
            if not eph["gps"] and not eph["glonass"]:
                raise RuntimeError("Р’ Р·Р°РіСЂСѓР¶РµРЅРЅС‹С… СЌС„РµРјРµСЂРёРґР°С… РЅРµ РЅР°С€Р»РѕСЃСЊ РЅРё РѕРґРЅРѕР№ Р·Р°РїРёСЃРё GPS/Р“Р›РћРќРђРЎРЎ вЂ” С„Р°Р№Р» РЅРµ РїРѕРґС…РѕРґРёС‚")
