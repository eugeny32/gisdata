...
(FACADEВ·CAD, TOPOВ·CAD, FACADEВ·FOTO вЂ” see ../../facade-cad/,
...
separate tables with no shared "person" concept вЂ” see the CHECK
...
FACADEВ·CAD/TOPOВ·CAD store drawn geometry as REAL PostGIS geometry (one row
per CAD object, not a JSON blob) вЂ” this is the one place in the whole
...
ST_GeomFromText/ST_AsGeoJSON needed in the session API views (Р¤Р°Р·Р° 6).
FACADEВ·FOTO has no such column: its control points live in photo pixel
...
class _SessionOwnerMixin(models.Model):
    """Shared owner fields вЂ” see module docstring for why not one polymorphic FK."""
...
class FacadeCadEntity(models.Model):
    ...
    # not geodetic вЂ” polyline -> LINESTRING as-is, circle/arc/ellipse/
...
class FacadeFotoSession(models.Model):
    admin = models.ForeignKey(Admin, on_delete=models.CASCADE, null=True, blank=True, related_name="+")
    user = models.ForeignKey(UserSync, on_delete=models.CASCADE, null=True, blank=True, related_name="+")
    # points/k1/k2/mmPerPx/target/source вЂ” same shape the tool already
