...
def tours_view(request):
    ...
    if request.method == "POST":
        ...
        if action == "save":
            ...
            if not name:
                error = "РЈРєР°Р¶РёС‚Рµ РЅР°Р·РІР°РЅРёРµ С‚СѓСЂР°"
            else:
                group_id = None
                if group_id_raw == "new":
                    if new_group_name:
                ...
                if uploaded:
                    ...
                    for up in uploaded:
                        ...
                        if ext not in ALLOWED_EXT:
                            error = f'РќРµРґРѕРїСѓСЃС‚РёРјС‹Р№ С„РѕСЂРјР°С‚ С„Р°Р№Р»Р° "{up.name}". Р Р°Р·СЂРµС€РµРЅРѕ: {", ".join(sorted(ALLOWED_EXT))}'
                            break
                    ...
                    for name1 in re.split(r"[\r\n]+", existing_file_name):
                        ...
                        if ext not in ALLOWED_EXT:
                            error = f'РќРµРґРѕРїСѓСЃС‚РёРјС‹Р№ С„РѕСЂРјР°С‚ С„Р°Р№Р»Р° "{name1}". Р Р°Р·СЂРµС€РµРЅРѕ: {", ".join(sorted(ALLOWED_EXT))}'
                            break
                        if not (UPLOAD_DIR() / name1).is_file():
                            error = f'Р¤Р°Р№Р» "{name1}" РЅРµ РЅР°Р№РґРµРЅ РІ uploads/tours/ РЅР° СЃРµСЂРІРµСЂРµ'
                            break
                        new_files.append({"file_path": name1, "file_format": ext})
                elif tour_id <= 0:
                    error = "Р”Р»СЏ РЅРѕРІРѕРіРѕ С‚СѓСЂР° РЅСѓР¶РЅРѕ Р·Р°РіСЂСѓР·РёС‚СЊ С„Р°Р№Р»(С‹) РјРѕРґРµР»Рё РёР»Рё СѓРєР°Р·Р°С‚СЊ РёРјРµРЅР° СѓР¶Рµ Р·Р°РіСЂСѓР¶РµРЅРЅС‹С… С„Р°Р№Р»РѕРІ"
            ...
            if not tour or not profile:
                error = "РўСѓСЂ РёР»Рё РїСЂРѕС„РёР»СЊ РїРѕРґРєР»СЋС‡РµРЅРёСЏ РЅРµ РЅР°Р№РґРµРЅ"
            else:
                local_file = UPLOAD_DIR() / tour.file_path
                try:
                    if not local_file.is_file():
                        raise RuntimeError(f"Р¤Р°Р№Р» РјРѕРґРµР»Рё РЅРµ РЅР°Р№РґРµРЅ РЅР° СЃРµСЂРІРµСЂРµ: {tour.file_path}")
    ...
    for t in tours:
        ...
        entry = tours_by_group.setdefault(key, {"name": t.group.name if t.group else "Р‘РµР· РіСЂСѓРїРїС‹", "tours": []})
    ...
    if request.method == "POST":
        ...
        if action == "add":
            ...
            if uploaded:
                ...
                if ext not in ALLOWED_EXT:
                    error = f"РќРµРґРѕРїСѓСЃС‚РёРјС‹Р№ С„РѕСЂРјР°С‚ С„Р°Р№Р»Р°. Р Р°Р·СЂРµС€РµРЅРѕ: {', '.join(sorted(ALLOWED_EXT))}"
                else:
                ...
                if ext not in ALLOWED_EXT:
                    ...
                    error = f'Р¤Р°Р№Р» "{existing_file_name}" РЅРµ РЅР°Р№РґРµРЅ РІ uploads/tours/ РЅР° СЃРµСЂРІРµСЂРµ'
                else:
                    file_path = existing_file_name
                    file_format = ext
            else:
                error = "Р—Р°РіСЂСѓР·РёС‚Рµ С„Р°Р№Р» РёР»Рё СѓРєР°Р¶РёС‚Рµ РёРјСЏ СѓР¶Рµ Р·Р°РіСЂСѓР¶РµРЅРЅРѕРіРѕ С„Р°Р№Р»Р°"
...
def tour_user_upload_view(request):
    ...
    if not name:
        return fail("РЈРєР°Р¶РёС‚Рµ РЅР°Р·РІР°РЅРёРµ")
    if lat == 0.0 and lon == 0.0:
        return fail("РќРµ СѓРєР°Р·Р°РЅС‹ РєРѕРѕСЂРґРёРЅР°С‚С‹ вЂ” РєР»РёРєРЅРёС‚Рµ РїСЂР°РІРѕР№ РєРЅРѕРїРєРѕР№ РїРѕ РєР°СЂС‚Рµ, С‡С‚РѕР±С‹ РІС‹Р±СЂР°С‚СЊ С‚РѕС‡РєСѓ")

    uploaded = request.FILES.getlist("model_files")
    if not uploaded:
        return fail("Р—Р°РіСЂСѓР·РёС‚Рµ С…РѕС‚СЏ Р±С‹ РѕРґРёРЅ С„Р°Р№Р» РјРѕРґРµР»Рё (.ply / .splat / .ksplat / .las)")

    total_size = sum(f.size for f in uploaded)
    if total_size > max_bytes:
        return fail(f"РЎСѓРјРјР°СЂРЅС‹Р№ СЂР°Р·РјРµСЂ С„Р°Р№Р»РѕРІ ({total_size / (1024**3):.2f} Р“Р‘) РїСЂРµРІС‹С€Р°РµС‚ Р»РёРјРёС‚ 2 Р“Р‘ РґР»СЏ РїРѕР»СЊР·РѕРІР°С‚РµР»СЊСЃРєРѕР№ Р·Р°РіСЂСѓР·РєРё")
    ...
    if plain_user_for_quota and not auth.current_admin(request):
        ...
        if quota is not None:
            used = user_storage_usage_bytes(plain_user_for_quota["id"])
            if used + total_size > quota:
                free = max(quota - used, 0)
                return fail(
                    f"РќРµРґРѕСЃС‚Р°С‚РѕС‡РЅРѕ РјРµСЃС‚Р° РІ С…СЂР°РЅРёР»РёС‰Рµ: РґРѕСЃС‚СѓРїРЅРѕ {free / (1024**3):.2f} Р“Р‘ РёР· "
                    f"{quota / (1024**3):.2f} Р“Р‘ (РёСЃРїРѕР»СЊР·РѕРІР°РЅРѕ {used / (1024**3):.2f} Р“Р‘)"
                )
    ...
    for up in uploaded:
        ext = up.name.rsplit(".", 1)[-1].lower() if "." in up.name else ""
        if ext not in ALLOWED_EXT:
            return fail(f'РќРµРґРѕРїСѓСЃС‚РёРјС‹Р№ С„РѕСЂРјР°С‚ С„Р°Р№Р»Р° "{up.name}". Р Р°Р·СЂРµС€РµРЅРѕ: {", ".join(sorted(ALLOWED_EXT))}')
...
def my_tours_view(request):
    ...
    if request.method == "POST":
        ...
        if not tour:
            error = "РўСѓСЂ РЅРµ РЅР°Р№РґРµРЅ РёР»Рё РЅРµ РїСЂРёРЅР°РґР»РµР¶РёС‚ РІР°Рј"
        elif action == "update":
            name = request.POST.get("name", "").strip()
            description = request.POST.get("description", "").strip()
            if not name:
                error = "РЈРєР°Р¶РёС‚Рµ РЅР°Р·РІР°РЅРёРµ"
            else:
                tour.name = name
                tour.description = description or None
    ...
    if not admin and plain_user:
        ...
        if storage_quota is not None:
            storage_used = user_storage_usage_bytes(plain_user["id"])
...
def tour_export_view(request):
    ...
    if not tour:
        return HttpResponseNotFound("РўСѓСЂ РЅРµ РЅР°Р№РґРµРЅ")
    ...
    if request.method == "POST":
        ...
        if action == "save":
            ...
            if not name or not host or not dbname or not username or port <= 0:
                error = "Р—Р°РїРѕР»РЅРёС‚Рµ РѕР±СЏР·Р°С‚РµР»СЊРЅС‹Рµ РїРѕР»СЏ: РЅР°Р·РІР°РЅРёРµ, С…РѕСЃС‚, РїРѕСЂС‚, Р±Р°Р·Р°, РїРѕР»СЊР·РѕРІР°С‚РµР»СЊ"
            else:
                if is_default:
                ...
                if conn_id > 0:
                    ...
                    if not password:
                        error = "РЈРєР°Р¶РёС‚Рµ РїР°СЂРѕР»СЊ РґР»СЏ РЅРѕРІРѕРіРѕ РїСЂРѕС„РёР»СЏ"
                    else:
            ...
            if not profile:
                test_result = {"ok": False, "name": "вЂ”", "message": "РџСЂРѕС„РёР»СЊ РЅРµ РЅР°Р№РґРµРЅ"}
            else:
                try:
                    conn = external_pg_connect(profile)
                    try:
                        ...
                        conn.close()
                    test_result = {"ok": True, "name": profile.name, "message": "РЎРѕРµРґРёРЅРµРЅРёРµ СѓСЃРїРµС€РЅРѕ СѓСЃС‚Р°РЅРѕРІР»РµРЅРѕ"}
                except Exception as exc:
    ...
    try:
        if action == "create_layer":
            ...
            if tour_id <= 0 or not name:
                raise ValueError("РЈРєР°Р¶РёС‚Рµ С‚СѓСЂ Рё РЅР°Р·РІР°РЅРёРµ СЃР»РѕСЏ")
            next_order = (
            ...
            if layer_id <= 0 or geom_type not in ("point", "polyline", "polygon") or not isinstance(coordinates, list) or not coordinates:
                raise ValueError("РќРµРєРѕСЂСЂРµРєС‚РЅС‹Рµ РґР°РЅРЅС‹Рµ Р°РЅРЅРѕС‚Р°С†РёРё")
            ...
            if anno_id <= 0:
                raise ValueError("РќРµРєРѕСЂСЂРµРєС‚РЅС‹Р№ id Р°РЅРЅРѕС‚Р°С†РёРё")
            fields = {}
            if "coordinates" in payload:
                if not isinstance(payload["coordinates"], list) or not payload["coordinates"]:
                    raise ValueError("РќРµРєРѕСЂСЂРµРєС‚РЅС‹Рµ РєРѕРѕСЂРґРёРЅР°С‚С‹")
            ...
            if not fields:
                raise ValueError("РќРµС‚ РїРѕР»РµР№ РґР»СЏ РѕР±РЅРѕРІР»РµРЅРёСЏ")
            TourAnnotation.objects.filter(id=anno_id).update(**fields)
            return JsonResponse({"ok": True})

        else:
            return JsonResponse({"error": "РќРµРёР·РІРµСЃС‚РЅРѕРµ РґРµР№СЃС‚РІРёРµ"}, status=400)
    except Exception as exc:
