...
class PgConnection(models.Model):
    ...
    # Plaintext by explicit design вЂ” see sql/schema.sql comment.
...
class Tour(models.Model):
    ...
    # is_enabled (which only governs visibility on the logged-in map) вЂ”
    ...
    # One tour per finished SLAM scan вЂ” set automatically by the SLAM
    # pipeline's final step (slam.services.link_tour(), Р¤Р°Р·Р° 7).
    slam_scan = models.OneToOneField(
        "slam.SlamScan", on_delete=models.SET_NULL, null=True, blank=True, related_name="tour",
    )
    created_at = models.DateTimeField(auto_now_add=True)
...
class TourAnnotation(models.Model):
    ...
    # JSON [[x,y,z], ...] in the tour model's LOCAL space (post-rotation) вЂ”
