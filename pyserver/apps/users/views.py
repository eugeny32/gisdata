    ...
    if request.method == "POST":
        ...
        if action == "add":
            ...
            if not user_name or not password:
                error = "РРјСЏ РїРѕР»СЊР·РѕРІР°С‚РµР»СЏ Рё РїР°СЂРѕР»СЊ РѕР±СЏР·Р°С‚РµР»СЊРЅС‹"
            elif UserSync.objects.filter(user_name=user_name).exists():
                error = f"РџРѕР»СЊР·РѕРІР°С‚РµР»СЊ В«{user_name}В» СѓР¶Рµ СЃСѓС‰РµСЃС‚РІСѓРµС‚"
            else:
                ...
                success = (
                    f"РџРѕР»СЊР·РѕРІР°С‚РµР»СЊ В«{user_name}В» СЃРѕР·РґР°РЅ (ID {user.id}) вЂ” С‚РѕР»СЊРєРѕ РІ "
                    "РїР»Р°С‚С„РѕСЂРјРµ (РЅР° Linux-СЃРµСЂРІРµСЂРµ Р·Р°РїРёСЃСЊ РІ mdb РЅРµРґРѕСЃС‚СѓРїРЅР°)"
                )
            ...
            password = request.POST.get("user_password", "").strip()
            if not password:
                error = "РџР°СЂРѕР»СЊ РЅРµ РјРѕР¶РµС‚ Р±С‹С‚СЊ РїСѓСЃС‚С‹Рј"
            else:
                user = UserSync.objects.filter(id=user_id).first()
                if not user or not user.is_manual:
                    error = "РЎРјРµРЅР° РїР°СЂРѕР»СЏ РґРѕСЃС‚СѓРїРЅР° С‚РѕР»СЊРєРѕ РґР»СЏ РїРѕР»СЊР·РѕРІР°С‚РµР»РµР№, РґРѕР±Р°РІР»РµРЅРЅС‹С… РІСЂСѓС‡РЅСѓСЋ"
                else:
                    user.user_password = password
                    user.save(update_fields=["user_password"])
                    success = "РџР°СЂРѕР»СЊ РѕР±РЅРѕРІР»С‘РЅ"
            ...
            user = UserSync.objects.filter(id=user_id).first()
            if not user or not user.is_manual:
                error = "РЈРґР°Р»СЏС‚СЊ РјРѕР¶РЅРѕ С‚РѕР»СЊРєРѕ РїРѕР»СЊР·РѕРІР°С‚РµР»РµР№, РґРѕР±Р°РІР»РµРЅРЅС‹С… РІСЂСѓС‡РЅСѓСЋ. MDB-РїРѕР»СЊР·РѕРІР°С‚РµР»Рё СѓРїСЂР°РІР»СЏСЋС‚СЃСЏ РІ РёСЃС‚РѕС‡РЅРёРєРµ."
            else:
                user.delete()
            ...
            try:
                ...
                success = f"РЎРёРЅС…СЂРѕРЅРёР·Р°С†РёСЏ Р·Р°РІРµСЂС€РµРЅР° вЂ” РѕР±РЅРѕРІР»РµРЅРѕ РёР· mdb-РґР°РјРїР°: {count} РїРѕР»СЊР·РѕРІР°С‚РµР»РµР№"
            except Exception as exc:
                error = f"РћС€РёР±РєР° СЃРёРЅС…СЂРѕРЅРёР·Р°С†РёРё РёР· mdb: {exc}"
