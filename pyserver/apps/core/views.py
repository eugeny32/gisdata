"""
Р¤Р°Р·Р° 2 vertical slice: auth pages + home dashboard вЂ” direct ports of
...
from apps.stations.models import Station, StationLog, StationStatus
...
def login_view(request):
    ...
    if request.method == "POST":
        ...
        if not user_name or not password:
            error = "Р’РІРµРґРёС‚Рµ РёРјСЏ РїРѕР»СЊР·РѕРІР°С‚РµР»СЏ Рё РїР°СЂРѕР»СЊ"
        else:
            user = auth.attempt_user_login(user_name, password)
            if user:
                auth.login_user_session(request, user)
                return redirect("home")
            error = "РќРµРІРµСЂРЅРѕРµ РёРјСЏ РїРѕР»СЊР·РѕРІР°С‚РµР»СЏ, РїР°СЂРѕР»СЊ РёР»Рё РґРѕСЃС‚СѓРї РёСЃС‚С‘Рє"
...
def admin_login_view(request):
    ...
    if request.method == "POST":
        ...
        if not login or not password:
            error = "Р’РІРµРґРёС‚Рµ Р»РѕРіРёРЅ Рё РїР°СЂРѕР»СЊ"
        else:
...
def employees_view(request):
    ...
    if request.method == "POST" and request.POST.get("action") == "toggle":
        ...
        if emp_id == admin["id"]:
            error = "РќРµР»СЊР·СЏ РґРµР°РєС‚РёРІРёСЂРѕРІР°С‚СЊ СЃРѕР±СЃС‚РІРµРЅРЅСѓСЋ СѓС‡С‘С‚РЅСѓСЋ Р·Р°РїРёСЃСЊ"
        else:
...
def invite_accept_view(request):
    ...
    if not invite:
        error = "РџСЂРёРіР»Р°С€РµРЅРёРµ РЅРµ РЅР°Р№РґРµРЅРѕ, СѓР¶Рµ РёСЃРїРѕР»СЊР·РѕРІР°РЅРѕ РёР»Рё РёСЃС‚С‘Рє СЃСЂРѕРє РґРµР№СЃС‚РІРёСЏ"
    ...
    if invite and request.method == "POST":
        ...
        if not login or not password:
            error = "Р—Р°РїРѕР»РЅРёС‚Рµ Р»РѕРіРёРЅ Рё РїР°СЂРѕР»СЊ"
