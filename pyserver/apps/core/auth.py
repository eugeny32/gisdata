"""
Dual-identity session auth вЂ” a direct port of app/lib/auth.php, not
...
PHP (`hash('sha256', $password)` вЂ” see admins table comment in
...
external Access DB (see users app) and is intentionally left as-is вЂ” it's
...
def attempt_user_login(user_name: str, raw_password: str) -> UserSync | None:
    ...
    # (see users/models.py) вЂ” compare_digest avoids a timing side-channel
