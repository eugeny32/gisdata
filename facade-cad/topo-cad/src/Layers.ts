...
/** Selection highlight — the UI accent, so "selected" reads the same in
 *  the drawing as it does on the ribbon. */
const SELECT_COLOR = '#f472b6';
...
export class LayerStore {
  ...
  private applyLayer(l: Layer): void {
    ...
    for (const m of [l.mat, l.matSel]) {
      // The drawing lives on the datum plane, metres BELOW the ground it
      // describes, so in the plan view the cloud would bury it. Linework is
      // drafting output — it always paints over the scan (together with the
      // renderOrder set in EntityStore.add).
      m.depthTest = false;
      m.linewidth = px;
