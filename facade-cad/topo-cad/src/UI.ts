 ...
 *
 * The cut pane owns ONE control strip (ПОПЕР ⇄ ПРОД · толщина · охват ·
 * вертикальный масштаб); the Engine swaps the numbers in it when the
 * orientation flips, so both cuts keep their own window.
 *
 * Pane labels double as the tracking indicator: a frozen pane (the user
 * grabbed it, or pressed 2 / 3) says so right in its caption.
 *
...
import type { SectionMode, ViewportName } from './ViewportManager';

const PANES: readonly ViewportName[] = ['main', 'section', 'iso'];

export interface UIHandlers {
  /** Ribbon height in px after every (re)layout — the viewports dock под ней. */
  ...
  onToggleSnap(): void;
  onToggleOrtho(): void;
  /** Flip the cut between ПОПЕРЕЧНИК and ПРОДОЛЬНИК. */
  onToggleSectionMode(): void;
  /** Slab thickness of the cut, metres. */
  onSectionThick(v: number): void;
  /** How much ground the cut spans on screen, metres. */
  onSectionSpan(v: number): void;
  /** Vertical exaggeration of the cut (×). */
  onSectionExag(v: number): void;
  /** Size of the cloud window that follows the cursor in 3D, metres. */
  onCursorArea(v: number): void;
  onToggleSlice(): void;
  onSliceZ(v: number): void;
  onSliceThick(v: number): void;
  onUcsSelect(index: number): void;
  onToggleLayer(name: ViewportName): void;
  onOpenCloudPanel(name: ViewportName): void;
  onCloudSetting(name: ViewportName, key: 'size' | 'opacity' | 'density' | 'mode', v: number): void;
  onToggleCloud(name: ViewportName): void;
...
export class UI {
  ...
  /** Pane caption text without the tracking suffix. */
  private readonly labelText: Record<ViewportName, string> = { main: 'ПЛАН', section: 'РЕЗ', iso: '3D · ОБЗОР' };
  private readonly labelFrozen: Record<ViewportName, boolean> = { main: false, section: false, iso: false };
  ...
  constructor(private readonly handlers: UIHandlers) {
    ...
    // Cut controls
    el('section-mode').addEventListener('click', () => handlers.onToggleSectionMode());
    const thick = el<HTMLInputElement>('sec-thick');
    thick.addEventListener('input', () => handlers.onSectionThick(parseFloat(thick.value)));
    const span = el<HTMLInputElement>('sec-span');
    span.addEventListener('input', () => handlers.onSectionSpan(parseFloat(span.value)));
    const exag = el<HTMLInputElement>('sec-exag');
    exag.addEventListener('input', () => handlers.onSectionExag(parseFloat(exag.value)));

    // 3D cloud window
    const area = el<HTMLInputElement>('cursor-area');
    area.addEventListener('input', () => {
      const v = parseFloat(area.value);
      el('cursor-area-val').textContent = `${v.toFixed(0)} м`;
      handlers.onCursorArea(v);
    });

    // Height slice over the plan
    el('slice-pill').addEventListener('click', () => handlers.onToggleSlice());
    const sliceZ = el<HTMLInputElement>('slice-z');
    sliceZ.addEventListener('input', () => handlers.onSliceZ(parseFloat(sliceZ.value)));
    const sliceT = el<HTMLInputElement>('slice-thick');
    sliceT.addEventListener('input', () => handlers.onSliceThick(parseFloat(sliceT.value)));
  ...
  private renderRibbon(): void {
    ...
    this.ribbon.forEach((g, gi) => {
      ...
      for (const id of g.buttons) {
        ...
        b.innerHTML = `<svg viewBox="0 0 24 24">${def.icon}</svg><span>${def.label}</span>`;
        tools.appendChild(b);
      }
  ...
  private cloudPanelFor: ViewportName | null = null;
  ...
  openCloudPanel(name: ViewportName, s: CloudSettingsLike): void {
    ...
    const titles: Record<ViewportName, string> = { main: 'ОБЛАКО · ПЛАН', section: 'ОБЛАКО · РЕЗ', iso: 'ОБЛАКО · 3D' };
    ...
    panel.style.top = name === 'iso' ? `${Math.max(8, r.top - panel.offsetHeight - 8)}px` : `${r.bottom + 8}px`;
  }
  ...
  setLayerVisible(name: ViewportName, on: boolean): void {
    el(`eye-${name}`).dataset.on = String(on);
  }

  setCloudVisible(name: ViewportName, on: boolean): void {
  ...
  /** Populate the axis dropdown; an empty list shows the world CS stub. */
  ...
  /** Cursor readout. In the axis UCS it is station / offset / ground
   *  elevation; in МСК it is the scan's own E / N / H. */
  setCoords(x: number, y: number, h: number | null, world: boolean): void {
    const he = h === null ? '   —  ' : h.toFixed(3);
    this.coords.textContent = world ? `E ${x.toFixed(3)}  N ${y.toFixed(3)}  H ${he}` : `X ${x.toFixed(3)}  Y ${y.toFixed(3)}  H ${he}`;
  }

  /** FPS plus how many cloud points the panes actually submitted — the
   *  tiling is invisible otherwise, and this is how you see it work. */
  ...
  setActivePane(name: string, css: { x: number; y: number; w: number; h: number }): void {
    ...
    for (const pane of PANES) {
      const lbl = document.getElementById(`lbl-${pane}`);
  ...
  setPaneLabel(name: ViewportName, text: string): void {
    this.labelText[name] = text;
    this.renderLabel(name);
  }

  /** A frozen pane says so: it stopped following the drafting cursor. */
  setPaneFrozen(name: ViewportName, frozen: boolean): void {
    if (this.labelFrozen[name] === frozen) return;
    this.labelFrozen[name] = frozen;
    this.renderLabel(name);
  }

  private renderLabel(name: ViewportName): void {
    const lbl = document.getElementById(`lbl-${name}`);
    if (!lbl) return;
    lbl.textContent = this.labelText[name];
    lbl.dataset.frozen = String(this.labelFrozen[name]);
    if (this.labelFrozen[name]) {
      const chip = document.createElement('span');
      chip.className = 'lbl-chip';
      chip.textContent = 'ФИКС';
      chip.title = 'Окно зафиксировано — клавиша 1 вернёт слежение за курсором';
  ...
  setSectionMode(mode: SectionMode): void {
    const pill = el('section-mode');
    pill.textContent = mode === 'cross' ? 'ПОПЕР' : 'ПРОД';
    pill.dataset.on = 'true';
    pill.title = mode === 'cross' ? 'Рез поперёк оси — нажмите, чтобы переключиться на продольный' : 'Рез вдоль оси — нажмите, чтобы переключиться на поперечный';
    // Windows differ by an order of magnitude between the two cuts.
    const span = el<HTMLInputElement>('sec-span');
    span.min = mode === 'cross' ? '2' : '10';
    span.max = mode === 'cross' ? '300' : '2000';
    span.step = mode === 'cross' ? '1' : '5';
  }

  setSectionThick(v: number): void {
    el<HTMLInputElement>('sec-thick').value = String(v);
    el('sec-thick-val').textContent = `${v.toFixed(2)} м`;
  }

  setSectionSpan(v: number): void {
    el<HTMLInputElement>('sec-span').value = String(v);
    el('sec-span-val').textContent = `${v.toFixed(v < 10 ? 1 : 0)} м`;
  }

  setSectionExag(v: number): void {
    el<HTMLInputElement>('sec-exag').value = String(v);
    el('sec-exag-val').textContent = `×${v % 1 === 0 ? v.toFixed(0) : v.toFixed(1)}`;
  }

  setCursorArea(v: number): void {
    el<HTMLInputElement>('cursor-area').value = String(v);
    el('cursor-area-val').textContent = `${v.toFixed(0)} м`;
  }

  showSectionPanels(on: boolean): void {
    for (const id of ['section-panel', 'cursor-panel', 'slice-panel']) {
      const p = el(id);
  ...
  /** Elevation range of the isolated area — the slider spans exactly it. */
  setSliceRange(min: number, max: number): void {
    const s = el<HTMLInputElement>('slice-z');
  ...
  /** Height-slice widgets: pill state + both sliders + readouts. */
  setSlice(on: boolean, z: number, thick: number): void {
    el('slice-pill').dataset.on = String(on);
    el<HTMLInputElement>('slice-z').value = String(z);
    el('slice-val').textContent = `${z.toFixed(2)} м`;
    el<HTMLInputElement>('slice-thick').value = String(thick);
    el('slice-thick-val').textContent = `${thick.toFixed(2)} м`;
  }
