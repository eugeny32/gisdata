...
export interface RawCloud {
  ...
  /** 0..255 на канал — нормализованный атрибут (см. ParsedCloud). */
  colors: Uint8Array;
  count: number;
...
/**
 * Потолок точек в памяти. С листовым индексом (CloudIndex) кадр платит
 * только за видимое, так что предел ставит не отрисовка, а память: точка
 * стоит ~15 байт в куче и столько же на GPU. deviceMemory — грубая, но
 * честная ручка Chromium: 8 ГБ → 64 млн (~2 ГБ суммарно), меньше — вдвое
 * и вчетверо. Файл крупнее читается прореженным шагом, как и раньше.
 */
const MEM_GB = (navigator as unknown as { deviceMemory?: number }).deviceMemory ?? 4;
/**
 * Потолок поднят этапом 3B (2026-08-07) — прежний (16/32/64М) был не
 * измерен, а взят «на глаз». Замер на 8 ГБ headless Edge: 100 млн точек —
 * холодный импорт 19 с, куча ГЛАВНОГО потока 870 МБ (честно, после
 * принудительной сборки мусора — до этого замер был обманут неубранным
 * старым облаком), FPS в обзоре 63 при бюджете 2.1М точек/кадр. Воркер
 * держит свою копию для лассо (~6 Б/точку, не видна в этом замере) и GPU —
 * свои VBO (~9 Б/точку); суммарно у 100 млн ~2.4 ГБ на все три места —
 * комфортно для машины с 8 ГБ. 120 млн проверены отдельно (см. память
 * проекта, ctf-run-and-test.md) — тот же профиль без роста кучи.
 */
export const MAX_POINTS = MEM_GB >= 8 ? 120_000_000 : MEM_GB >= 4 ? 40_000_000 : 16_000_000;
...
function recordToPoint(v: DataView, base: number, h: LasHeader, pos: Float32Array, col: Uint8Array, w: number): void {
  ...
  pos[w * 3 + 2] = -y;
  if (h.rgbAt >= 0) {
    const r = v.getUint16(base + h.rgbAt, true);
    const g = v.getUint16(base + h.rgbAt + 2, true);
    const b = v.getUint16(base + h.rgbAt + 4, true);
    // 16-битные каналы → старший байт; 8-битные лежат в младшем как есть.
    const s = r > 255 || g > 255 || b > 255 ? 8 : 0;
    col[w * 3] = r >> s;
    col[w * 3 + 1] = g >> s;
    col[w * 3 + 2] = b >> s;
  } else {
