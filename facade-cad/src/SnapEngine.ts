 ...
 *
 * A uniform grid over UCS-local XY is used instead of an octree: the
 * isolated wall is a 2.5-D slab, so 2D binning gives O(1) neighborhood
 * queries with better cache behavior and far less code.
 *
 ...
 * the arithmetic — was the visible stall when a wall was isolated on a slow
 * machine. Two linear passes over typed arrays allocate nothing per point.
 *
 * `query` returns the REAL 3-D point (z ≠ 0). The caller projects it onto
 * the UCS plane (z := 0) so the drawing stays flat, while the green marker
 * is rendered at the true point.
 */
import * as THREE from 'three';

const _p = new THREE.Vector3();

const CELL = 0.35; // meters; ≈ snap radius at typical drafting zoom
const GRID_MAX = 2048; // per-axis cell cap — bounds the index for huge walls
...
export class SnapEngine {
   ...
   *  the compact wall buffer in local coordinates anyway, so handing that
   *  array over here saves a second full transform pass over the wall. */
  ...
  private finishBuild(pts: Float32Array, n: number, minX: number, minY: number, maxX: number, maxY: number): void {
    ...
    this.ox = minX;
    this.oy = minY;
    this.cols = Math.max(1, Math.min(GRID_MAX, Math.ceil((maxX - minX) / CELL) + 1));
    this.rows = Math.max(1, Math.min(GRID_MAX, Math.ceil((maxY - minY) / CELL) + 1));
  ...
  private cellIndex(x: number, y: number): number {
    const cx = Math.min(this.cols - 1, Math.max(0, Math.floor((x - this.ox) / CELL)));
    const cy = Math.min(this.rows - 1, Math.max(0, Math.floor((y - this.oy) / CELL)));
    return cy * this.cols + cx;
  }

  /** Nearest indexed point to (x, y) within `maxR` (all UCS-local, screen
   *  distance ≈ XY distance because the main view is plane-aligned).
   *  `zMin/zMax` restrict candidates to a depth slab — with the facade
   *  slice active the cursor snaps to the visible reveal, not to points
   *  hidden behind it. Allocation-free: runs on every mousemove. */
  query(x: number, y: number, maxR: number, out: THREE.Vector3, zMin = -Infinity, zMax = Infinity): boolean {
    ...
    const cr = Math.min(4, Math.ceil(maxR / CELL));
