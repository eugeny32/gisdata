...
export interface ToolHost {
  ...
  snapOn: boolean;
  orthoOn: boolean;
  /** Depth window for cloud snapping (±Infinity when the facade slice is off). */
  readonly snapZMin: number;
  readonly snapZMax: number;
  /**
   * Точки стены в осях ПСК, парами [x, y] — вход для поиска проёмов.
   * Глубина берётся из среза фасада, а без среза — автоматически по самому
   * плотному слою: искать дырки имеет смысл в ОДНОЙ плоскости, иначе окно
   * дальней стены закроет проём ближней.
   */
  wallPointsXY(): { xy: Float32Array; n: number } | null;
  /** Дешёвая проверка «есть ли что искать» — без прохода по облаку. */
  hasWall(): boolean;
  /**
   * Навести окно черчения на прямоугольник в осях ПСК и увести туда же
   * слежение соседних окон. Размер нужен, чтобы подобрать масштаб: показать
   * область целиком и с запасом, а не упереться в неё вплотную.
   */
  ...
  /** Цветовой фильтр слоя поиска проёмов/рельефа (панель «ЦВЕТ»). */
  colorFilter(): { on: boolean; tol: number; count: number; css: string | null };
  setColorFilter(on: boolean, tol: number): void;
  /** Пипетка: цвет точки под лучом → образец; css для квадратика в панели. */
  addColorSample(ray: THREE.Ray): string | null;
  clearColorSamples(): void;
  /** ОБЛАСТЬ поиска: окно 1 показывает только её; null — весь фасад. */
  setSearchRegion(r: { x0: number; y0: number; x1: number; y1: number } | null): void;
  defineUcs(p1: THREE.Vector3, p2: THREE.Vector3): void;
  enterTopView(): void;
  /** Side view of the isolated wall — where the plan slice level is set. */
  enterLevelView(): void;
  /** Move the plan slice to this world height (click in the side view). */
  ...
  snapshot(): void;
  /** Depth (UCS-local z) of the facade reference plane, null when unset. */
...
export abstract class CadTool {
  ...
  /**
   * Причина, по которой инструмент сейчас нельзя включить, или null.
   *
   * Спрашивается ДО постановки. Раньше инструмент отказывался уже из
   * activate(), переключая менеджер на «Выбор», — а тот, вернувшись из
   * вызова, объявлял активным всё равно себя и затирал сообщение об отказе.
   * Состояние расходилось с показанным: подсвечена одна кнопка, работает
   * другая.
   */
  canArm(): string | null {
    return null;
...
export class ToolManager {
  ...
  private static readonly DRAFT_IDS = new Set(['line', 'polyline', 'rect', 'circle', 'arc', 'ellipse', 'text', 'point', 'refplane', 'deviation', 'move', 'copy', 'rotate', 'scale', 'mirror', 'trim', 'join', 'openings']);

  /**
   * Регистрация инструмента снаружи — так подключаются отделяемые модули
   * (например «Проёмы»). Ядро о них не знает: нет вызова — нет инструмента,
   * и ни один import не тянет модуль в сборку ядра.
   */
  addTool(t: CadTool): void {
    this.tools.set(t.id, t);
  }
  ...
  activate(id: string): void {
    ...
    const why = next.canArm();
    if (why) {
      this.host.ui.setHint(why);
      return;
    }
  ...
  private computePointer(e: PointerEvent, vp: Viewport): void {
    ...
    // В МСК ПСК формально определена (матрица последней стены жива), но
    // проецировать клики на ЕЁ плоскость нельзя: лассо следующего фасада
    // схлопывалось бы в линию прежней стены. canDraft() ложен ровно в МСК —
    // там указатель живёт в мировых осях, как до первой ПСК.
    const localMode = ucs.defined && this.host.canDraft();
    if (localMode) {
    ...
    // hit keeps its real z for the marker; drawing always flattens to z=0.
    if (this.host.snapOn && localMode && vp.name !== 'profile') {
    ...
    if (localMode) ucs.localToWorld(p.local, p.world);
    ...
    if (localMode) this.cross.position.set(p.local.x, p.local.y, 0.001);
   ...
   *  viewport (perspective uses distance-based world-per-pixel). */
  applyViewportScale(vp: Viewport): void {
    ...
    if (cam instanceof THREE.PerspectiveCamera) {
      const d = this.cross.getWorldPosition(_num).distanceTo(cam.position);
      wpp = (2 * d * Math.tan((cam.fov * Math.PI) / 360)) / vp.css.h;
    } else {
      wpp = this.host.vpm.worldPerPixel(vp);
    }
    if (this.marker.visible) this.marker.scale.setScalar(8 * wpp);
    if (this.cross.visible) this.cross.scale.setScalar(9 * wpp);
  }
...
/** Top-down lasso fence: filters the cloud to one wall (GPU-side). */
class IsolateTool extends CadTool {
  readonly id = 'isolate';
  readonly hint = 'ШАГ 1/4 · СТЕНА — обведите стену контуром на виде сверху: ЛКМ вершины · Enter/ПКМ применить · Esc сброс';
  ...
  override onFinish(): void {
    ...
    this.reset();
    // Изоляция асинхронна (математика в воркере): к «уровню» переключит
    // Engine.onLassoResult, когда стена реально готова.
  }
...
/** Between the lasso and the UCS: the isolated wall seen from the SIDE,
 *  where the height of the plan slice is chosen. A top view of the whole
 *  wall is roof + ground + vegetation on top of each other; a 20 cm slab
 *  at window-sill height is a clean line you can actually click along. */
class LevelTool extends CadTool {
  readonly id = 'level';
  readonly hint = 'ШАГ 2/4 · УРОВЕНЬ СРЕЗА — вид сбоку: кликните по высоте (или ползунки H и Т), затем «К ПЛАНУ» / Enter';
  ...
  override onDown(p: DraftPointer): void {
    // The drafting plane stands vertical in this view, so the hit point's
    // height is exactly the level the user pointed at.
    this.host.setPlanSliceLevel(p.world.y);
  }

  override onFinish(): void {
    this.mgr.activate('ucs'); // back to the plan, now sliced
  }

  override onKey(e: KeyboardEvent): boolean {
    if (e.key === 'Escape') {
      this.mgr.activate('isolate'); // re-cut the area
      return true;
    }
    return false;
  }
...
/** Two clicks on the isolated wall define a new wall UCS. */
class UcsTool extends CadTool {
  readonly id = 'ucs';
  readonly hint = 'ШАГ 3/4 · ПСК — кликните 2 точки вдоль линии стены в срезе (ось X пойдёт по ним) · Esc заново';
  ...
  constructor(host: ToolHost, mgr: ToolManager) {
    super(host, mgr);
    this.seg = [this.p1];
    this.rb = new RubberBand(host.scene, 0x22d3ee);
  }
...
class PolylineTool extends CadTool {
  ...
  readonly hint = 'ПОЛИЛИНИЯ — ЛКМ точки по фасаду · Enter/ПКМ завершить · C замкнуть · Esc отмена';
  private readonly pts: THREE.Vector3[] = [];
  private readonly rb: RubberBand;
...
class TextTool extends CadTool {
  ...
  override onDown(p: DraftPointer): void {
    ...
    if (text && text.trim()) {
      this.host.snapshot();
      this.host.store.add(new TextEntity(p.local, text.trim(), 0.35));
      this.mgr.activate('select');
    }
  }
}
...
/** Pick the reference plane for facade deviations: it is parallel to the
 *  wall UCS and passes through the picked scan point, so "zero" is a real
 *  place on the building rather than an abstract offset. */
class RefPlaneTool extends CadTool {
  readonly id = 'refplane';
  readonly hint = 'ПЛОСКОСТЬ — кликните точку фасада (удобно в 3D-окне): она станет нулём для отклонений';
...
/** Signed distance from the reference plane, written next to a point.
 *  Plus = outward (towards the viewer), minus = into the wall. */
class DeviationTool extends CadTool {
  readonly id = 'deviation';
  readonly hint = 'ОТКЛОНЕНИЕ — кликайте точки фасада: рядом появится подпись + наружу / − внутрь · Esc выход';

  override activate(): void {
    this.host.enterDraftView();
    if (this.host.deviationRef === null) {
      this.host.ui.setHint('Сначала задайте нулевую плоскость: инструмент ПЛОСКОСТЬ, клик по фасаду');
  ...
  override onDown(p: DraftPointer): void {
    ...
    if (ref === null) return;
    // p.snapLocal carries the REAL scan depth; with no snap there is no
