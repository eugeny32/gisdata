...
export const MAT_GHOST_LINE = new THREE.LineBasicMaterial({ color: 0x8b9cf9, transparent: true, opacity: 0.4 });
export const MAT_GHOST_LINE2 = new LineMaterial({ color: 0x8b9cf9, transparent: true, opacity: 0.4, linewidth: 1.6 });
export const MAT_GHOST_MESH = new THREE.MeshBasicMaterial({ color: 0x8b9cf9, transparent: true, opacity: 0.25, side: THREE.DoubleSide, depthWrite: false });
...
export class TextEntity extends CadEntity {
  ...
  constructor(
    ...
    this.material = new THREE.MeshBasicMaterial({ map: tex, transparent: true, side: THREE.DoubleSide, depthWrite: false });
  ...
  protected applyLayer(l: Layer): void {
    this.material.color.set(this.selected ? '#38bdf8' : l.color);
  }
...
const PT_R = 0.035; // marker half-size, metres — a real drawing symbol

/** Survey point: a small diamond marker with an optional caption.
 *  Facade deviation readings are exactly this — a point whose caption is the
 *  signed distance from the reference plane (see Engine.deviationRef). */
export class PointEntity extends LineworkEntity {
  ...
  private buildLabel(): void {
    ...
    this.labelMat = new THREE.MeshBasicMaterial({ map: tex, transparent: true, side: THREE.DoubleSide, depthWrite: false });
  ...
  protected override applyLayer(l: Layer): void {
    super.applyLayer(l);
    this.labelMat?.color.set(this.selected ? '#38bdf8' : l.color);
  }
...
export class EntityStore {
  ...
  /**
   * ПСК, активная ПРЯМО СЕЙЧАС — Engine держит её здесь же зеркалом своего
   * `activeUcs` (−1 = МСК). Новая сущность рождается на ЭТОЙ ПСК; каждая
   * стена в документе — своя система, и добавленная точка чертежа обязана
   * запомнить, на какой из них она возникла (см. homeUcs / add()).
   */
  activeUcsIndex = -1;
  /** ПСК-«дом» каждой сущности — на какой стене она нарисована. Каждая
   *  стена в документе живёт в своих координатах (`points`, z всегда 0,
   *  локально к СВОЕЙ ПСК); общего 3D-пространства без искажений между
   *  стенами нет — общий рендер-узел ucs.group несёт трансформ ОДНОЙ, той,
   *  что сейчас активна. Поэтому сущности чужой ПСК просто ПРЯЧУТСЯ, а не
   *  пересчитываются: попытка «дотащить» их до новой плоскости через
   *  applyMatrix4+сплющивание z портила бы их НАВСЕГДА (было устройство
   *  EntityStore.rebase — снято именно из-за этого: обвели фасад 1, ушли
   *  строить фасад 2 на новой ПСК, вернулись — фасад 1 оказывался
   *  расплющен на плоскость фасада 2). */
  private readonly homeUcs = new Map<CadEntity, number>();

  constructor(private readonly parent: THREE.Group) {}

  /** `homeUcs` — обычно опущен (берётся activeUcsIndex); restore() передаёт
   *  ЯВНО сохранённое значение, а не текущую ПСК — иначе отмена действия,
   *  сделанного на другой стене, перетасовала бы «домашность» сущностей. */
  add(e: CadEntity, homeUcs: number = this.activeUcsIndex): void {
    ...
    this.homeUcs.set(e, homeUcs);
    // Сразу подчиняем видимость правилу «своя ПСК видна, чужая скрыта» —
    // иначе точка, добавленная во время смены ПСК (проёмы), мелькнула бы.
    e.object3D.visible = homeUcs === this.activeUcsIndex && homeUcs >= 0;
    if (e.object3D.visible) e.refreshAppearance();
    this.onChange?.();
  }
  ...
  /**
   * Смена активной ПСК: показать сущности, «родные» для неё, спрятать
   * остальные. `activeUcsIndex` — то же число, что и Engine.activeUcs
   * (−1 = МСК, там не видно ничего: МСК — обзор ОБЛАКА, не чертежа).
   * Скрытые сущности не трогаются — они останутся ровно теми же, когда их
   * ПСК будет выбрана снова: см. комментарий у `homeUcs` про то, почему
   * когда-то стоявший здесь rebase() портил геометрию другой стены.
   */
  refreshUcsVisibility(activeUcsIndex: number): void {
    this.activeUcsIndex = activeUcsIndex;
    for (const e of this.entities) {
      const mine = activeUcsIndex >= 0 && this.homeUcs.get(e) === activeUcsIndex;
      if (mine) e.refreshAppearance(); // сама решит по слою
      else e.object3D.visible = false;
    }
    // Выделение чужой (только что спрятанной) сущности — источник багов у
    // инструментов правки, которые её больше не видят и не ждут.
    for (const e of [...this.selection]) {
      if (!e.object3D.visible) this.toggle(e);
    }
  }

  // ------------------------------------------------- undo/redo snapshots

  /** Обёртка вокруг toJSON() несёт homeUcs — своя для КАЖДОЙ сущности, а
   *  не общая: отмена действия на другой стене не должна путать «домашность». */
  serialize(): string {
    return JSON.stringify(this.entities.map((e) => ({ u: this.homeUcs.get(e) ?? -1, j: e.toJSON() })));
  }

  restore(json: string): void {
    this.clearAll();
    const list = JSON.parse(json) as { u: number; j: EntityJSON }[];
    for (const o of list) {
      const e = entityFromJSON(o.j);
      // ЯВНО передан сохранённый homeUcs, а не текущая ПСК — иначе Ctrl+Z
      // действия, сделанного на другой стене, переписало бы её «домашность».
      if (e) this.add(e, o.u);
    }
  }
